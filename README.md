...
- `scan_engine.py` / `nse_trade_graph.py` — a typed NSE Scan Engine whose production adapter runs four independent, differently-failing checks over one symbol, an explicit list, or an index.
...
## NSE Scan Engine (`scan_engine.py`, `nse_trade_graph.py`)

The caller interface is `ScanEngine.scan(ScanRequest) -> ScanResult`. Batch scans,
intraday rechecks, and fixture tests use that same typed seam; mutable stage state and
string transitions are private to the production adapter. A result carries disposition,
typed reason codes, policy/model identity, total and per-stage timing, decision ID, and
durable failure details. The engine proposes—but never executes—an NSE stock trade,
gated behind four independent checks:
...
fetch -> technical -> [REJECT] -> abort (no retry -- see below)
          ...
          sentiment -> [REVIEW] -> flag_review (manual review, no proposal)
           ...
           propose (proposal text only — no broker call, ever)
              |
              v
          persist (Scan Engine writes JSONL + evaluation ledger)
```
...
- **fundamental** — fetched only for symbols that pass the technical gate, avoiding unnecessary NSE calls for every technical rejection. `fundamentals.get_fundamental_snapshot` pulls corporate announcements, corporate actions, yearwise returns, and peer comparison data via NSE's `NextApi`, then slowly fetches up to six standalone integrated-financial XBRL filings. `financial_results.py` selects the newest revision per period and normalizes separate profiles for banks, NBFCs, and non-financial companies. Deterministic policy checks profile-specific multi-period coverage, earnings, margins, leverage/cash conversion or asset quality before any model call. NSE's shallow quote shareholding response is supplemented by Regulation 31 XBRL history warmed into Aerospike: FII, DII, government, promoter, and other-public percentages are derived from reconciled share counts across five consecutive quarters. QoQ and four-quarter changes plus trend labels are deterministic for all five categories. The model sees only bounded announcement/corporate-action facts with stable evidence IDs; it does not receive or calculate financial ratios, shareholding trends, delivery data, or final policy. Its cited IDs are schema-checked, and invalid output fails closed to `REVIEW`. A final `PASS` means only that no material red flag was found in supplied policy-required evidence—not that the company is generally “good.” Source ages are calculated as of the scan date; stale peer/shareholding data and missing profile fields force `REVIEW` without model inference.
...
Every terminal record carries a typed `disposition` (`PROPOSE`, `REVIEW`, `REJECT`, or infrastructure-only `FAILED`) plus `decision_reason.stage` and `decision_reason.code`. `REJECT` always maps to the non-actionable `aborted` status; `REVIEW` remains `flagged_for_review`. Each record also persists a versioned decision graph at meaningful seams—data validity, technical families, fundamental coverage, qualitative evidence, risk, disposition, alert eligibility, and outcome eligibility. Indicators remain evidence inside the technical-family node rather than becoming runtime nodes. Digests render terminal fields directly, and the evaluation ledger groups reason codes without parsing verdict prose.
...
Every completed scan still appends JSONL, and now also inserts the same record into an immutable SQLite decision ledger (`EVALUATION_DB_PATH`, default `evaluation.db`, gitignored). Unhandled stage failures become typed `FAILED` results and durable failure records instead of escaping as missing decisions. Batch membership and per-symbol completion/failure events also append to `SCAN_RUN_LOG_PATH` (default `scan_runs.jsonl`) so requested symbols remain auditable when SQLite is unavailable. On the next scan, unfinished journal runs older than `SCAN_RUN_STALE_AFTER_SECONDS` (default six hours) are closed as `RecoveredIncompleteScan`; the timeout avoids interfering with active overlapping scans. The decision record includes the effective backend/model/token limit (local, Anthropic, or stub), `NSE_POLICY_VERSION`, stage timings, and the decision graph, so later model, prompt, scoring and risk-policy changes remain distinguishable. `evaluation.py report` joins completed outcomes back to technical-family direction, fundamental coverage, qualitative interpretation, and risk-node status through `decision_graph_performance`. Because calibration selects one candidate per symbol/date/policy, bump `NSE_POLICY_VERSION` when the model, prompt, scoring, or risk configuration changes. A SQLite ledger failure does not fail the scan when JSONL succeeded: JSONL is the durable recovery path and decision records can be imported idempotently.
...
- **`scan_label`** — every `ScanRequest` supplies its label (`NSE_SCAN_LABEL`, default `manual`, is only the batch CLI default). This is how `digest.py`/`intraday_recheck.py` find “this run’s” records without relying on calendar-date matching. Intraday rechecks pass their label directly and no longer mutate the environment before importing the scanner.
